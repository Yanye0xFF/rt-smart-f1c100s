...
static tina_ccu_t CCU = RT_NULL;

void ccu_init(void) {
    CCU = (tina_ccu_t)platform_get_periph_vaddr(F1C100S_CCU_BASE);
}
...
/**
 * @brief 获取外设PLL频率,应当固定返回600M(手册要求),否则请检查时钟配置
 * @return periphal clk should fix at 600MHz
 * */
...
int cpu_get_clk(void) {
	...
	if (cpusrc == CLK_PLL_SRC)
		...
		return _32KHZ_ / (HCLKC_DIV(reg) + 1);  //内部32KHz时钟 猜的
...
rt_err_t cpu_set_pll_clk(int clk) {
	...
	CCU->pll_cpu_ctrl = (0x1 << 31) | (m << 0) | (k << 4) | (n << 8)
			| (p << 16);
...
rt_err_t video_set_pll_clk(int clk) {
	...
	m = (m - 1) & 0xf;
	//speed = (24*n)/m
	CCU->pll_video_ctrl = (0x1 << 31) | (0x0 << 30) | (0x1 << 24) | (n << 8)
			| m;
...
rt_err_t periph_set_pll_clk(int clk) {
	...
	if ((clk > 1800000000) || (clk < 200000000) || (clk == 0)
			|| (clk % _24MHZ_ != 0))
		return RT_EINVAL;
...
rt_err_t mmc_set_clk(enum mmc_clk_id clk_id, int hz) {
	...
	volatile rt_uint32_t *mmc_clk =
			(clk_id == SDMMC0) ? (&CCU->sdmmc0_clk) : (&CCU->sdmmc1_clk);
	...
	*mmc_clk = (0x1 << 31) | pll | (sclk_dly << 20) | (n << 16)
			| (oclk_dly << 8) | (div - 1);
