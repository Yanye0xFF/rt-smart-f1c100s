...
int arch_user_space_init(struct rt_lwp *lwp)
{
    ...
    // 高1G的地址空间(0xC0000000~0xFFFFFFFF)是内核独享的, 用户程序只能使用低3G内存, TLB拷贝12K就足够
...
void arch_kuser_init(rt_mmu_info *mmu_info, void *vectors)
{
    ...
    int kuser_sz = __kuser_helper_end - __kuser_helper_start; // 0xa0 = 160

    void *addr = rt_hw_mmu_map_auto(mmu_info, vectors, 0x1000, MMU_MAP_U_RO);

    rt_memcpy((void*)((char*)addr + 0x1000 - kuser_sz), __kuser_helper_start, kuser_sz);
    /*
