    ...
    for (i = 0; i < eheader.e_phnum; i++, off += sizeof pheader)
    {
        ...
        if (pheader.p_type == PT_LOAD)
        {
            ...
            if (pheader.p_filesz < pheader.p_memsz)
            {
                ...
                off2 = pheader.p_filesz & ARCH_PAGE_MASK;
                va = (void *)((pheader.p_vaddr + pheader.p_filesz + load_off) & ~ARCH_PAGE_MASK);
                while (size)
                {
                    size_s = (size < ARCH_PAGE_SIZE - off2) ? size : ARCH_PAGE_SIZE - off2;
                    pa = rt_hw_mmu_v2p(m_info, va);
                    va_self = (void *)((char *)pa - PV_OFFSET);
                    memset((void *)((char *)va_self + off2), 0, size_s);
                    rt_hw_cpu_dcache_ops(RT_HW_CACHE_FLUSH, (void *)((char *)va_self + off2), size_s);
                    off2 = 0;
                    size -= size_s;
                    va = (void *)((char *)va + ARCH_PAGE_SIZE);
                }
