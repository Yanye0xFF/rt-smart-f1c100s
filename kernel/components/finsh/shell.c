...
static rt_bool_t shell_handle_history(struct finsh_shell *sh)
{
#if defined(_WIN32)
    int i;
    rt_kprintf("\r");
    ...
    rt_kprintf("\033[2K\r");
#endif
    rt_kprintf("%s%s", FINSH_PROMPT, sh->line);
    return RT_FALSE;
}

static void shell_push_history(struct finsh_shell *sh)
{
    if (sh->line_position != 0)
    {
        /* push history */
        if (sh->history_count >= FINSH_HISTORY_LINES)
        {
            /* if current cmd is same as last cmd, don't push */
            if (memcmp(&sh->cmd_history[FINSH_HISTORY_LINES - 1], sh->line, FINSH_CMD_SIZE))
            {
                /* move history */
                int index;
                for (index = 0; index < FINSH_HISTORY_LINES - 1; index ++)
                {
                    memcpy(&sh->cmd_history[index][0],
                           &sh->cmd_history[index + 1][0], FINSH_CMD_SIZE);
                }
                memset(&sh->cmd_history[index][0], 0, FINSH_CMD_SIZE);
                memcpy(&sh->cmd_history[index][0], sh->line, sh->line_position);

                /* it's the maximum history */
                sh->history_count = FINSH_HISTORY_LINES;
            }
        }
        else
        {
            /* if current cmd is same as last cmd, don't push */
            if (sh->history_count == 0 || memcmp(&sh->cmd_history[sh->history_count - 1], sh->line, FINSH_CMD_SIZE))
            {
                sh->current_history = sh->history_count;
                memset(&sh->cmd_history[sh->history_count][0], 0, FINSH_CMD_SIZE);
                memcpy(&sh->cmd_history[sh->history_count][0], sh->line, sh->line_position);
